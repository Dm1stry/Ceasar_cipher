...
{
    shift_label_->setBuddy(shift_spin_box_);  //Makes mnemonical binding possible
    ...
    Encoder * ceasar_encoder = new Encoder(shift_spin_box_->value(), encoded_text_edit_);  //Creates encoder filter, that retranslate encoded input to encoded_text_edit_
    source_text_edit_->installEventFilter(ceasar_encoder);  //Sets filter on keyboard input for source text
    ...
    QObject::connect(shift_spin_box_, SIGNAL(valueChanged(int)), ceasar_encoder, SLOT(shiftChanged(int)));  //Connects value in spinbox with shift of encoding
    shift_spin_box_->setValue(3);  //Sets default value
